...
void PlayerController::Update()
{
	...
	if (IsKeyRelease(VK_SPACE)) {
		...
		trans->SetAngle({
			...
			GetOwner()->GetComponent<Transform>()->GetAngle().y + 180.0f,// 矢のモデルと矢を射出するモデルの正面が違う場合、ここで数値調整する。
			GetOwner()->GetComponent<Transform>()->GetAngle().z
			});
		// "Arrow"という名前のオブジェクトのRigidbodyを取得
